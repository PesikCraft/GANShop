/* sync-bridge v2.8 (ES5)
 * Надёжная синхронизация каталога/заказов/банка/пользователей без правок app.js.
 * - Edit-lock каталога: 60s после лок. правок pull не перетирает их.
 * - X-Shop-Ts + обработка 409 — защита от «старых» записей.
 * - Зеркала ключей, пересчёт категорий, без переупорядочивания продуктов.
 */
(function(){
  ...
  // Сколько держать лок после правки каталога (чтобы pull не откатывал)
  ...
  // ---- Безрекурсивные setItem/removeItem ----
  var mute={};
      ...
      if (group==='catalog'){
        ...
        req('PUT','/api/catalog',{products:products,cats:cats},{'X-Shop-Ts': String(ts)}).then(function(r){
          if (!r.ok && r.status===409 && r.data){
            // Сервер считает наш апдейт старым → подтягиваем актуальное
            ...
            // Не ломаем порядок: берём как есть
  ...
  localStorage.setItem=function(key,value){
    ...
    if (!isMuted(key)){
      ...
      // попытка аутентификации (для админа) — без влияния на каталог
  ...
  function grantAdminSession(u){
    var nick=uname(u)||'admin';
    var ustr=JSON.stringify({ nick:nick, role:'admin', ts:now() });
    setLS('is_admin','true'); setLS('isAdmin','true'); setLS('admin','true');
    setLS('role','admin'); setLS('user_role','admin');
  ...
  // ---- PULL с учётом edit-lock ----
  function applyServerCatalog(products){
    // Не меняем порядок, принимаем как есть
  ...
  function pullOnce(){
    ...
    req('GET','/api/catalog').then(function(r){
      ...
      if (isEditLocked()){
        // в режиме редактирования не перетираем локальное
        return;
      }
  ...
  function startPolling(){
    setInterval(function(){
      ...
      // catalog
      if (ok('catalog')) pullOnce();
      else {
        // даже если dirty, всё равно поддерживаем orders/bank/users
        req('GET','/api/orders').then(function(r){
          var d=r.data||{}, orders=Array.isArray(d&&d.orders)?d.orders:[];
          if (!same(parseArr(localStorage.getItem(KEY_ORDERS)),orders)) setLS(KEY_ORDERS, j(orders));
          mirrorFromPrimary();
  ...
  function startWatch(){
    ...
    setInterval(function(){
      ...
      list=SYN_USERS_STATIC.concat(keysByPrefix('shop_users_')).concat(keysByPrefix('users_')).concat(keysByPrefix('accounts_')); for(i=0;i<list.length;i++) trackKey(list[i],'users');

      for(i=0;i<LOGIN_NICK_KEYS.length;i++) trackKey(LOGIN_NICK_KEYS[i],null);
      for(i=0;i<LOGIN_PASS_KEYS.length;i++) trackKey(LOGIN_PASS_KEYS[i],null);
    }, WATCH_INTERVAL_MS);
  }
